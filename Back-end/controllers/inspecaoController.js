...
exports.getInspecoes = async (req, res) => {
  try {
    const inspecoes = await Inspecao.find().populate('user').populate('reciclagem');
    res.json(inspecoes);
  } catch (error) {
...
exports.getInspecaoById = async (req, res) => {
  try {
    const inspecao = await Inspecao.findById(req.params.id).populate('user').populate('reciclagem');
    if (!inspecao) {
